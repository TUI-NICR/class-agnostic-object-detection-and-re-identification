...
if False:
    ...
    with open("co3d_reid_v1_bb_stats.json", "w") as f:
        json.dump(stats, f)


with open("co3d_reid_v1_bb_stats.json", "r") as f:
    stats = json.load(f)
...
ax[0].hist2d(img_sizes[0], img_sizes[1], bins=100)
...
ax[1].hist(img_side_ratios, bins=50)
ax[1].set_title("Image Aspect Ratios")

ax[2].hist2d(bb_sizes[0], bb_sizes[1], bins=100)
ax[2].set_title("BB Sizes")

ax[3].hist(np.log2(bb_side_ratios), bins=50)
ax[3].set_xticks(ax[3].get_xticks(), np.exp2(ax[3].get_xticks()))
ax[3].set_title("BB Aspect Ratios")

ax[4].hist(np.log2(img_bb_area_ratios), bins=50)
ax[4].set_xticks(ax[4].get_xticks(), np.exp2(ax[4].get_xticks()))
ax[4].set_title("Image-BB Area Ratios")

ax[5].hist2d(np.log2(img_bb_size_ratios[0]), np.log2(img_bb_size_ratios[1]), bins=100)
...
fig.savefig("co3d_v1_bb_props.pdf")
