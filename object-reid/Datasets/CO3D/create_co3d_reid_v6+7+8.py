...
for split in ["train", "query", "test"]:
    ...
    os.makedirs("co3d_reid_v6", exist_ok=True)
    df_100.to_csv(f"co3d_reid_v6/{split}.csv", index=False)
    os.makedirs("co3d_reid_v7", exist_ok=True)
    df_075.to_csv(f"co3d_reid_v7/{split}.csv", index=False)
    os.makedirs("co3d_reid_v8", exist_ok=True)
    df_050.to_csv(f"co3d_reid_v8/{split}.csv", index=False)
