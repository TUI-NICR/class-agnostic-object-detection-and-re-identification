...
def main(model='vitl14', dataset='fs_coco17_support_novel_30shot', use_bbox='yes',
    ...
    #model = torch.hub.load("/home/user/.cache/torch/hub/facebookresearch_dinov2_main", 'dinov2_' + model, source="local")
    ...
    with tqdm(total=epochs * len(dataloader)) as bar:
        for _ in range(epochs):
            for item_i, item in enumerate(dataloader):
                ...
                start = time()
                r = model.get_intermediate_layers(image14x[None, ...], return_class_token=True, reshape=True)    
                ...
                if 'stuff' in dataset_name:
                    ...
                    if use_bbox:
                        bbox_masks = to_mask(instances.gt_boxes.tensor, instances.image_size[1], instances.image_size[0]).to(device)
                        #assert bbox_masks.shape == masks_used.shape
                        masks_used = bbox_masks
                    else:
                        masks_used = instances.gt_masks.tensor
                    for bmask, label, bbox in zip(masks_used, instances.gt_classes, instances.gt_boxes.tensor):
                        start = time()
