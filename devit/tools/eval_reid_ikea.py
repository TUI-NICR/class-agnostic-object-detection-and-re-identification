...
def make_gallery_dataset(filter_imgs=None):
    def return_func():
        P_GT = "/path/to/paper/ikea-benchmark/gallery_ikea.json"
        with open(P_GT, "r") as f:
            input_gts = json.load(f)
        P = "/path/to/ikea-dataset/IKEA_ASM/ANU_ikea_dataset_images/"
        dataset = []
        anns = defaultdict(list)
        for cat in input_gts["categories"]:
            cat["id"] -= 1
...
def make_query_dataset(filter_imgs=None):
    def return_func():
        P_GT = "/path/to/paper/ikea-benchmark/queries_ikea_new.json"
        with open(P_GT, "r") as f:
            comparison_gts = json.load(f)
        P = "/path/to/paper/ikea-benchmark/queries_ikea_new"
        dataset = []
...
def make_query_dataset_with_masks(filter_imgs=None):
    def return_func():
        P = "/path/to/paper/ikea-benchmark/queries_ikea_detectron_new"
        P_GT = "/path/to/paper/ikea-benchmark/queries_ikea_detectron_new.json"
        with open(P_GT, "r") as f:
            comparison_gts = json.load(f)
        dataset = []
...
def main(args):
    ...
    cfg.OUTPUT_DIR = cfg.OUTPUT_DIR.replace("shot-5", f"shot-{NUM_SHOTS}_split-{SPLIT}{'_sw' if SLIDING_W else ''}").replace("few-shot", "few-shot_ikea2")
    print(f"Number of Shots: {NUM_SHOTS}")
    default_setup(cfg, args)
    TMP_OUT = TemporaryDirectory()  # tmp_outputs

    DatasetCatalog.register("full_gallery_dataset_ikea", make_gallery_dataset())
    ...
    DatasetCatalog.register("full_query_dataset", make_query_dataset_with_masks(
        [f"{x+y}.png" for x in range(0, 300, 20) for y in range(0, 20, 20 // NUM_SHOTS)]
    ))
    with open("/path/to/paper/ikea-benchmark/gallery_ikea.json", "r") as f:
        input_gts = json.load(f)
    ...
    MetadataCatalog.get("full_gallery_dataset_ikea").thing_classes = classes
    MetadataCatalog.get("full_gallery_dataset_ikea").thing_colors = colors
    ...
    cfg.DATASETS.TRAIN = ("coco_plus_ikea",)
    cfg.DATASETS.TEST = ("full_gallery_dataset_ikea",)
    cfg.freeze()
    ...
    evaluator = COCOEvaluator(
        "full_gallery_dataset_ikea", output_dir=TMP_OUT.name, few_shot_mode=True,
        seen_cnames=[], unseen_cnames=classes, all_cnames=classes
    )

    if SLIDING_W:
        dataloader = build_detection_test_loader(cfg, sliding_window("full_gallery_dataset_ikea", 512), mapper=SlidingWindowMapper(cfg))
    else:
        dataloader = build_detection_test_loader(cfg, "full_gallery_dataset_ikea")
    results = []
    evaluator.reset()
    timing = 0
        ...
        for inp, res in results:
            for i, cls_ in enumerate(classes):
                ...
                vis = Visualizer(
                    ...
                    MetadataCatalog.get("full_gallery_dataset_ikea")
                )
