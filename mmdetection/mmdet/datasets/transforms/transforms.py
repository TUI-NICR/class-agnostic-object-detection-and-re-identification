...
class Crop(BaseTransform):
    ...
    def _crop_data(self, results, allow_negative_crop=False):
        """Function to crop images, bounding boxes, masks, semantic
        segmentation maps.
        ...
        offset_w = self.crop_x1
        offset_h = self.crop_y1
        ...
        if results.get('gt_bboxes', None) is not None:
            ...
            if results.get('gt_masks', None) is not None:
                results['gt_masks'] = results['gt_masks'][
                    valid_inds.nonzero()[0]].crop(
                        np.asarray([self.crop_x1, self.crop_y1, self.crop_x2, self.crop_y2]))
                if self.recompute_bbox:
                    results['gt_bboxes'] = results['gt_masks'].get_bboxes(
                        type(results['gt_bboxes']))
        ...
        # crop semantic seg
        if results.get('gt_seg_map', None) is not None:
            results['gt_seg_map'] = results['gt_seg_map'][self.crop_y1:self.crop_y2,
...
@TRANSFORMS.register_module()
class CropAttachFront(BaseTransform):
    """Random crop the image & bboxes & masks.
