...
class DetLocalVisualizer(Visualizer):
    ...
    def _draw_instances(self, image: np.ndarray, instances: ['InstanceData'],
        ...
        if 'masks' in instances:
            ...
            self.draw_binary_masks(masks, colors=cmap, alphas=self.alpha)
    ...
    def add_datasample(
        ...
        if draw_pred and data_sample is not None:
            ...
            crop_size_0 = 1040
            crop_size_1 = 1700
            crop_y1, crop_y2 = offset_h, offset_h + crop_size_0
            crop_x1, crop_x2 = offset_w, offset_w + crop_size_1
            pred_img_data = pred_img_data[crop_y1:crop_y2, crop_x1:crop_x2, ...]
            gt_img_data = pred_img_data
            image = pred_img_data
            """
        ...
        if out_file is not None:
            img_id = out_file.split('/')[-1]
            x = data_sample.metainfo
            img_path = x['img_path']
            shape = x['ori_shape']
            hxw = str(shape[1]) + 'x' + str(shape[0])
            split = img_path.split('/')[-2]
            new_id = hxw + '/' + split + '/' + img_id
            out_file_new = out_file.replace(img_id, new_id)
            if 'e4sm' in img_path:
                id = str(data_sample.img_id) + '.jpg'
                out_file_new = out_file.replace(img_id, id)
            mmcv.imwrite(drawn_img[..., ::-1], out_file_new)
        else:
