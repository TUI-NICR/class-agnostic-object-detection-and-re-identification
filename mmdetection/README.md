# Class-Agnostic Object Detection with MMDetection
This repo is cloned from [MMDetection on GitHub](https://github.com/open-mmlab/mmdetection.git) and adjusted slightly for class-agnostic object detection on the ATTACH dataset.
...
For running inference on the whole dataset:
```bash
conda install tqdm
...
For running the deployed onnx model create a new environment
```bash
...
## Training
```bash
python tools/train.py configs/rtmdet/rtmdet-ins_tiny_attach.py
```

When new labeled data are available, the data needs to be converted to COCO format, as done here. \
Then, the paths to the `ann_file`s need to be adjusted in the config [`configs/rtmdet/rtmdet-ins_tiny_attach.py`](configs/rtmdet/rtmdet-ins_tiny_attach.py) accordingly.

## Testing
```bash
export WORK_DIR=/path/to/work_dir
python tools/test.py \
    $WORK_DIR/rtmdet-ins_tiny_attach.py \
    $WORK_DIR/epoch_12.pth \
    --show \
    --show-dir $WORK_DIR/predictions_visualized
```

## Inference on whole dataset for training action recognition
During preprocessing, images are cropped first using the config file [`configs/rtmdet/rtmdet-ins_tiny_attach_cropped_inference.py`](configs/rtmdet/rtmdet-ins_tiny_attach_cropped_inference.py).
```bash
python tools/inference_whole_dataset.py \
    --dataset attach \
    --images-or-video-path /path/to/raw_attach_dataset/color \
    --batch-size 16 \
    --checkpoint-filepath /path/to/checkpoints/epoch_12.pth \
    --cfg configs/rtmdet/rtmdet-ins_tiny_attach_cropped_inference.py \
    --threshold 0.1 \
    --output-path /path/to/predictions_rtmdet_cropped \
    --start-index-video 0 \
    --show
```

## Deployment
Deploy model to onnx with:
```bash
export WORK_DIR=/path/to/config
...
python tools/deploy.py \
    ...
    $WORK_DIR/rtmdet-ins_tiny_attach.py \
    $WORK_DIR/epoch_12.pth \
    /path/to/attach/tapes/01__1__shuttleFront/e4sm_tape-2021-06-30_15-19-01.380105_png.shuttleFront/shuttleFront_Kinect_ColorImage1625059151346781000.jpg \
    --work-dir $WORK_DIR/deploy
```
...
```bash
conda activate onnx
python tools/inference_onnx_rtmdet.py \
    --dataset attach \
    --onnx-filepath $WORK_DIR/deploy/end2end.onnx \
    --img-or-video-filepath /path/to/Attach/raw_attach_dataset/color/29__2__shuttleFront/e4sm_tape-2021-07-20_15-31-35.523041_png.shuttleFront_color.mp4 \
